...
export default withRouter(function ManageStudentPage({ history }) {
    ...
    useEffect(() => {
        apis.studentAdminController.getStudentList(page).then(response => {
        ...
        function processFile(file) {
            ...
            reader.onload = function () {
                ...
                if (window.confirm(`총 ${result.split('\n').length - 1}명을 업로드 하시겠습니까?\n${result}`)) {
                    ...
                    apis.studentAdminController.uploadCsv(result).then(response => {
    ...
    return (
        <Container fluid className="main-content-container px-4">
            ...
            <Row>
                {showUploadWaiting ? <div style={{ position: 'fixed', zIndex: 999, backgroundColor: 'rgba(0,0,0,0.5)', left: 0, top: 0, width: '100%', height: '100%' }} >Upload waiting</div> : undefined}
                <Col md={selectedUser ? "5" : "12"}>
                    <Card small className="mb-4">
                        <CardHeader className="border-bottom">
                            ...
                            <Button className="float-right" onClick={() => { setShowAddUserModal(true) }}>교육생 추가</Button>
                            <Button className="float-right" style={{ marginRight: 10 }} onClick={() => { if (fileInputRef && fileInputRef.current) { fileInputRef.current.click() } }}>교육생 CSV 추가</Button>
                            <a href="/student_csv_example.csv" download><Button className="float-right" style={{ marginRight: 10 }} >교육생 CSV 추가 샘플</Button></a>
                            <a href="/api/v1/admin/student/csv" download><Button className="float-right" style={{ marginRight: 10 }}>교육생 리스트 다운로드</Button></a>
                        </CardHeader>
                        <CardBody>
                            <table className="table mb-0">
                                ...
                                <tbody>
                                    {
                                        studentPageData &&
                                            studentPageData.content &&
                                            studentPageData.content.length > 0 ?
